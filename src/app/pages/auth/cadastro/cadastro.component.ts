...
export class cadastroComponent {
  ...
  // ===== CPF =====
  cpfDigits = '';   // só dígitos (11)
  cpfMasked = '';   // exibição 000.000.000-00
  ...
  onNomeInput(ev: Event) {
    ...
    this.nome = (input.value || '').replace(/[^A-Za-zÀ-ÖØ-öø-ÿ\s'-]/g, '').replace(/\s{2,}/g, ' ');
  ...
  // ===== Form ok =====
  get formValid() {
    return this.cpfValido && this.nomeOk && this.lenOk && this.upperOk && this.numberOk && this.specialOk && this.matchOk;
  }
  ...
  voltar() {
    // se sua rota do login for outra, altere aqui
